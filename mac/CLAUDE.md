...
- When a logical unit of work is done, give me the commit as plain text. Use `Changed files:` (repo-relative paths) then `Git commit message:` with a flush-left subject + body matching the project's existing style (conventional commits, etc.). No fenced code block, no `git commit`/HEREDOC wrapper.
...
- Never use em-dashes (`—`) in replies or in files you write. Use commas, colons, parentheses, or sentence breaks instead. Plain hyphens (`-`) are fine.
